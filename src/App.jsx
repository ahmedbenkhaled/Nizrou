  ...
  const handleProtectedAction = (actionType, marketId, option = "YES") => {
    if (!user) {
      ...
      if (actionType === "TRADE") {
        ...
        alert(`✨ تم تعديل حالة هذا الصوت [رقم: ${marketId}] في قائمتك المفضلة بنجاح!`);
      }
    }
  };
  ...
  useEffect(() => {
    const checkGeoblocking = async () => {
      try {
        ...
        // حفظ الحالة المكتشفة موحدة كأحرف صغيرة للـ UI والـ Navbar
        setDetectedCountry(countryCode.toLowerCase());

        // جلب إعدادات الحظر الجغرافي بأمان
        ...
        if (data && data.length > 0) {
          ...
          // معالجة وتنظيف عناصر المصفوفة القادمة من قاعدة البيانات لضمان دقة المطابقة
          const normalizedBlockedCountries = Array.isArray(blocked_countries)
            ? blocked_countries.map(c => String(c).toUpperCase())
            : [];

          // التوجيه القسري لصفحة الحظر في حال تحقق الشروط بأحرف موحدة كبيرة
          if (vpn_blocking_enabled && normalizedBlockedCountries.includes(countryCode.toUpperCase())) {
