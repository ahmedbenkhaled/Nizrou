...
let currentCurrency = "USDT";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" dir="rtl">
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-6 text-white relative shadow-2xl font-cairo">
      ...
      {method === "binance" 
        ...
        : "طلبك الآن قيد المراجعة من قبل الإدارة، سيتم تحديث رصيدك فور الموافقة."}
            ...
            <button 
              ...
              className="w-full mt-4 py-2.5 bg-slate-850 border border-slate-700 rounded-xl hover:bg-slate-800 transition-all font-bold text-sm"
            >
              إغلاق النافذة
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* اختيار وسيلة الشحن */}
            <div>
              <label className="block text-xs font-semibold text-slate-400 mb-2">اختر وسيلة الإيداع:</label>
              <div className="grid grid-cols-2 gap-3">
                {/* 1. زر بايننس الثابت لجميع دول العالم دون استثناء */}
                <button
  type="button"
  onClick={() => setMethod("binance")}
  className={`py-3 px-4 rounded-xl border text-sm font-bold flex flex-col items-center justify-center gap-2 transition-all ${
    method === "binance" 
...
>
  ...
  <span>بايننس (Binance Pay)</span>
    ...
    className={`py-3 px-4 rounded-xl border text-sm font-bold flex flex-col items-center justify-center gap-2 transition-all ${method === "baridimob" ? "border-blue-500 bg-blue-500/10 text-blue-400" : "border-slate-800 bg-slate-950/40 text-slate-400 hover:text-white"}`}
  >
    ...
    <span>بريدي موب (Baridimob)</span>
                ...
                {detectedCountry === "ma" && (
                  <button
                    ...
                    className={`py-3 px-4 rounded-xl border text-sm font-bold flex flex-col items-center justify-center gap-1 transition-all ${method === "cih_bank" ? "border-emerald-500 bg-emerald-500/10 text-emerald-400" : "border-slate-800 bg-slate-950/40 text-slate-400 hover:text-white"}`}
                  >
                ...
                {detectedCountry === "tn" && (
                  <button
                    ...
                    className={`py-3 px-4 rounded-xl border text-sm font-bold flex flex-col items-center justify-center gap-1 transition-all ${method === "sobflous" ? "border-purple-500 bg-purple-500/10 text-purple-400" : "border-slate-800 bg-slate-950/40 text-slate-400 hover:text-white"}`}
                  >
            ...
            <div className="p-3 bg-slate-950/60 border border-slate-850 rounded-xl text-xs space-y-1 text-slate-300">
              {method === "binance" && (
                <div className="space-y-4 flex flex-col items-center text-center p-2">
                  ...
                  <div className="w-full space-y-2">
                    <p className="text-xs font-bold text-yellow-500">📥 محفظتك الرقمية الشخصية لاستقبال الإيداع (USDT - Polygon):</p>
                    <div className="bg-slate-950 border border-slate-800 p-2.5 rounded-xl text-xs font-mono select-all break-all text-center tracking-wider text-emerald-400">
              ...
              {method === "baridimob" && (
                <>
                  <p className="font-bold text-amber-400 mb-1">معلومات حساب بريدي موب للجزائر:</p>
                  <p>الإسم: AHMED BENKHALED</p>
                  <p className="font-mono select-all bg-slate-900 p-1 rounded mt-1 text-center text-sm tracking-wider text-white">RIP: 00799999000123456789</p>
                </>
              )}

              {method === "cih_bank" && (
                <>
                  <p className="font-bold text-amber-400 mb-1">معلومات الحساب البنكي للمغرب (CIH):</p>
                  <p>الإسم: MOHAMED ALAMI</p>
                  <p className="font-mono select-all bg-slate-900 p-1 rounded mt-1 text-center text-sm tracking-wider text-white">RIB: 230123456789012345678901</p>
                </>
              )}

              {method === "sobflous" && (
                <>
                  <p className="font-bold text-amber-400 mb-1">معلومات الدفع لتونس (Sobflous):</p>
                  <p>رقم المحفظة أو المعرّف:</p>
                  <p className="font-mono select-all bg-slate-900 p-1 rounded mt-1 text-center text-sm text-white">ID: 5543219</p>
                </>
              )}
            </div>
  ...
  المبلغ المراد شحنه ({method === "binance" ? "بالدولار USDT" : method === "baridimob" ? "بالدينار الجزائري" : method === "cih_bank" ? "بالدرهم المغربي" : "بالدينار التونسي"}):
              ...
              <input
                ...
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 text-center font-bold"
              />
            </div>

            {/* رفع صورة الوصل */}
            {/* رفع صورة الوصل - يظهر فقط للطرق المحلية */}
{method !== "binance" && (
  <div>
    <label className="block text-xs font-semibold text-slate-400 mb-1">إرفاق صورة إثبات التحويل (الوصل):</label>
    <input
      type="file"
      accept="image/*"
      required={method !== "binance"}
      onChange={handleFileChange}
      className="w-full text-xs text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-slate-800 file:text-white hover:file:bg-slate-700 file:cursor-pointer bg-slate-950 border border-slate-800 p-1.5 rounded-xl"
    />
  </div>
)}

           {error && <p className="text-xs text-red-400 bg-red-500/10 p-2 rounded-lg text-center font-semibold">{error}</p>}

            {/* زر الإرسال الذكي الجديد */}
            <button
              type="submit"
